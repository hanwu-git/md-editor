...
本地 Markdown 编辑器桌面应用，分屏实时预览、Mermaid 流程图、代码高亮、查找替换、多编码支持、明暗主题、视图开关。**完全本地运行，无需联网。**
...
| 功能 | 说明 |
|---|---|
| 基础编辑 | 仿 Windows 记事本：新建 / 打开 / 保存 / 另存为 / 最近打开（持久化，跨会话保留），未保存关闭有确认提示 |
...
| 文件属性 FileVersion / ProductVersion | 1.1.2 |
| 应用窗口标题栏 | 未命名 - MD编辑器 v1.1.2 |
| 缓存目录名 | `%LOCALAPPDATA%\MD编辑器\app-1.1.2` |
